...
def download_from_gdrive(dst_root, fn, gdrive_path, iszip=True):
  ...
  if not os.path.exists("{}/{}".format(dst_root, fn)):
    ...
    if iszip:
        unzip('{}.zip'.format(fn), args.dataroot)
    else:
        shutil.move(fn,   args.dataroot)
...
for fn in tqdm(os.listdir(args.dataroot + "/images")):
  ...
  if new_fn[:-4] in target_fns:
    shutil.move(args.dataroot + "\\images\\"+fn,   args.dataroot + "\\test\\"+new_fn)
  else:
    shutil.move(args.dataroot + "\\images\\"+fn,   args.dataroot + "\\train\\"+new_fn)
